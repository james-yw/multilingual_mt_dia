...
restore_path=/home/wyang/work/multilingual_mt_dia/models/mt_many_to_many/checkpoint50.pt
...
restore_file="--restore-file $restore_path"
model_args="$arch $extra_arch $restore_file"

# task args
task="--user-dir ../utils --task my_dialogue"
task_args="$task --source-lang src --target-lang tgt"

task_mt="--user-dir ../utils --task my_translation_multi_simple_epoch"
task_mt_args="$task_mt --lang-pairs $lang_pairs --langs $langs_comma --encoder-langtok tgt"
...
for num in 100; do
    ...
    checkpoint="checkpoint$num" # checkpoint... or checkpoint_last or checkpoint_best
    ...
    for lang in ${LANG_PAIR[@]}; do
        ...
        fairseq-generate $data_args $task_mt_args $decoding_args --source-lang $src --target-lang $tgt > $result_path/generate-test-${checkpoint}.txt
